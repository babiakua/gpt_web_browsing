...
class GPTAnswer:
    ...
    def _format_reference(self, relevant_docs_list, link_list):
        ...
        for i in range(self.TOP_K):
            ...
            formatted_reference += (reference_content_list[i] + '\n\n\n')
    ...
    def get_answer(self, query, relevant_docs, output_format, company, year):
        # Create an instance of ChatOpenAI and generate an answer
        llm = ChatOpenAI(model_name=self.model_name, openai_api_key=self.api_key, temperature=0.0, max_tokens=4095)
        ...
        summary_prompt = prompt_template.format(context_str=relevant_docs, query=query, format=output_format, comp=company, year=year)
        # print("\n\nThe message sent to LLM:\n", summary_prompt)
        # print("\n\n", "="*30, "GPT's Answer: ", "="*30, "\n")
        gpt_answer = llm([HumanMessage(content=summary_prompt)])
        # gpt_answer = model.generate_content(summary_prompt).text
        return gpt_answer
